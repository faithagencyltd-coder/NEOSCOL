...
export const EDITABLE_FORMS = {
  ...
  guardian: { title: "Fiche parent / tuteur", description: "Champs complémentaires de la fiche des parents et tuteurs." },
} as const;
...
/** Champs personnalisés actifs d'un formulaire (fiche élève, fiche parent…). */
export async function getActiveFormFields(organizationId: string, kind: EditableFormKind) {
  const supabase = await createClient();
  const { data } = await supabase.from("form_definitions").select("fields").eq("organization_id", organizationId).eq("kind", kind).eq("is_active", true).maybeSingle();
