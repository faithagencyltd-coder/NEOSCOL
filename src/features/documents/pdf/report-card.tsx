...
const creditLabel = (n: number) => (Number.isInteger(n) ? String(n) : String(n).replace(".", ","));
...
export function ReportCardPage({
  ...
  if (credits) columns.push({ label: "Crédits", width: "7%", align: "center" });
  ...
  return (
    <Page size="A4" style={styles.page}>
      ...
      <InfoGrid
        rows={[
          [v.student, `${student.last_name} ${student.first_name}`],
          ["Matricule", student.matricule],
          [v.klass, snapshot.class_name],
          ["Né(e) le", `${pdfDate(student.birth_date)}${student.birth_place ? ` à ${student.birth_place}` : ""}`],
          [v.headTeacher, snapshot.head_teacher],
      ...
      <View style={{ flexDirection: "row", gap: 10, marginTop: 10 }} wrap={false}>
        ...
        {credits ? (
          <View style={[styles.box, { flex: 1 }]}>
            <Text style={{ color: COLORS.muted }}>Crédits ECTS acquis</Text>
            <Text style={{ fontFamily: "Helvetica-Bold", fontSize: 18, color }}>
              {creditLabel(credits.earned)} / {creditLabel(credits.total)}
            </Text>
            <Text style={styles.small}>Acquise si la moyenne atteint {pdfNumber(credits.threshold, 0)}/20</Text>
          </View>
        ) : null}
        {showRank ? (
          <View style={[styles.box, { flex: 1 }]}>
      ...
      <View style={{ marginTop: 10, gap: 6 }} wrap={false}>
        {card.head_teacher_comment ? (
          <View style={styles.box}>
            <Text style={styles.bold}>Appréciation : {v.headTeacher.toLowerCase()}</Text>
            <Text>{pdfText(card.head_teacher_comment)}</Text>
          </View>
        ) : null}
        {card.appreciation ? (
          <View style={styles.box}>
            <Text style={styles.bold}>{v.family === "school" ? "Appréciation du conseil de classe" : "Appréciation du jury"}</Text>
