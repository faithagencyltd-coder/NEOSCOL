...
test("catalogue des permissions synchronisé (TypeScript ↔ base)", async () => {
  ...
  // Tri côté JS des deux listes : indépendant de la collation de la base.
  const fromDb = await as(null, async (q) => (await q("select code from permissions")).map((r) => r.code).sort());
